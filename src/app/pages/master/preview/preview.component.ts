...
export class PreviewComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    // this.occupiedWidth = totalWidth;
    // if (specificRow) {
    //   const rowWidthInPixels = specificRow.offsetWidth;

    //   // Set the specificRowWidth property to be used as needed
    //   this.dynamicTableWidth = rowWidthInPixels + 'px';

    // Manually trigger change detection
    this.cdr.detectChanges();
    // }

    // const secondTableElement = this.firstTable
    //   .nativeElement as HTMLTableElement;
    // secondTableElement.style.width = this.dynamicTableWidth + 'px';
    // this.cdr.detectChanges();
  }
  ngOnDestroy(): void {
    this.cdr.detectChanges();
  ...
  toolTipStyle(type, i, j) {
    ...
    if (type == 'body') {
      ...
      // else if(type=='footer'){
      //
      //   this.getToolTip.footer.forEach((e) => {
      //     if (j['day'] == e.footerHeader) {
      //
      //       this.toolBool = true;
      //       this.tooltipPush.push({
      //         FilledBy: e.filledBy,
      //         FilledAt: e.filledAt
      //       });
      //     }
  ...
  getTemplateData() {
    this.dataService
      ...
      .subscribe((res) => {
        ...
        this.tempHeading1.forEach((e) => {
          if (e.periodic == false) {
            this.periodicPush.push(e.periodic);
            this.periodicLength = this.periodicPush;
          }
        });
  ...
  getMasterData() {
    ...
    this.dataService
      ...
      .subscribe((res) => {
        ...
        this.masterName = this.previewData.masterName;
        ...
        if (!this.footerHead?.length) {
          ...
          // this.footerHead =this.previewData.footer.length? Object.keys(this.previewData.footer[0]):[];
        }
